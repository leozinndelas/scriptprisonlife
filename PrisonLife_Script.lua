...
do
    ...
    MakeButton(p, "🔫 Pegar Todas as Armas", "TP para o armeiro e coleta armas do mapa", 4, function()
        pcall(function()
            -- Primeiro TP para perto do armeiro
            local hrp = GetHRP()
            if hrp then
                hrp.CFrame = CFrame.new(835, 99, 2256)
            end
            task.wait(0.5)

            -- Lista de nomes de armas válidas do Prison Life
            local weaponNames = {"Remington 870", "M9", "AK-47", "M4A1", "Riot Shield", "Glock", "Taser", "Handcuffs", "Keycard", "Shotgun", "Pistol"}

            -- Pegar somente armas do Workspace que NÃO estão dentro de um personagem de jogador
            for _, item in ipairs(Workspace:GetDescendants()) do
                if item:IsA("Tool") then
                    -- Verificar se NÃO está dentro do personagem de nenhum jogador
                    ...
                    for _, pl in ipairs(Players:GetPlayers()) do
                        ...
                        if pl.Backpack and item:IsDescendantOf(pl.Backpack) then
                            isPlayerTool = true
                            break
                        end
                    end
                    if not isPlayerTool then
                        pcall(function() item.Parent = Player.Backpack end)
                    end
                end
            end
        end)
        Notify("Armas coletadas! (sem roubar de jogadores)")
    end)
...
FloatBtn.MouseEnter:Connect(function()
    Tween(FloatBtn, {Size = UDim2.new(0, 50, 0, 50), BackgroundColor3 = C.Accent3}, 0.2)
end)
FloatBtn.MouseLeave:Connect(function()
    Tween(FloatBtn, {Size = UDim2.new(0, 44, 0, 44), BackgroundColor3 = C.Accent}, 0.2)
end)

-- Funções de abrir/fechar UI
